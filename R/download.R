...
execute_download <- function(url, dest_file, parallel_n = 0, software_paths) {
  ...
  if (parallel_n > 0 && !is.null(software_paths$axel)) {
    ...
    run_command(software_paths$wget,
                ...
                spinner = FALSE) # Wget has its own progress
  } else {
...
get_aspera_key_path <- function(db = "ENA", software_paths) {
  ...
  if (db == "ENA") {
    ...
    if (is.null(key_path)) {
      cli::cli_alert_warning("Could not find standard ENA Aspera key. Set {.code options({option_name} = ...)} if needed.")
    }
  } else { # GSA
    ...
    if (is.null(key_path)) {
      cli::cli_alert_warning("Could not find or download GSA Aspera key. Set {.code options({option_name} = ...)} if needed.")
    }
  }
  return(key_path)
}
...
#' @return A list containing paths to downloaded files (or the SRA file path). Keys: `files`, `type` ("fastq_gz", "sra").
#' @keywords internal
download_ena_sra_run <- function(run_id, run_info, download_fastq_gz = TRUE, use_aspera = FALSE,
  ...
  if (download_fastq_gz && database_preference == "ena" && "fastq_ftp" %in% names(run_info)) {
    fastq_ftp_links <- str_split(run_info$fastq_ftp, ";")[[1]] %>% discard(is.na) %>% discard(. == "")
    if (use_aspera && "fastq_aspera" %in% names(run_info)) {
      fastq_aspera_links <- str_split(run_info$fastq_aspera, ";")[[1]] %>% discard(is.na) %>% discard(. == "")
      # Ensure ftp and aspera links correspond if possible (e.g., same number)
      if(length(fastq_aspera_links) != length(fastq_ftp_links)) {
        cli::cli_alert_warning("Mismatch between number of FTP ({length(fastq_ftp_links)}) and Aspera ({length(fastq_aspera_links)}) FASTQ links for {.val {run_id}}. Preferring FTP.")
    ...
    for (i in seq_along(links_to_use)) {
      ...
      tryCatch({
        if (download_mode == "Aspera") {
          ...
          execute_download(url = paste0("https://", link), dest_file = dest_file,
  ...
  if (!is.null(ena_sra_ftp_link)) {
    ...
    tryCatch({
      if (download_mode == "Aspera") {
        execute_aspera(remote_path = link_to_use, db = "ENA", software_paths = software_paths,
                       aspera_key_path = aspera_key_path, output_dir = output_dir)
        ...
        execute_download(url = paste0("https://", link_to_use), dest_file = dest_file,
                         parallel_n = parallel_n, software_paths = software_paths)
      }
      if (fs::file_exists(dest_file) && fs::file_size(dest_file) > 0) {
        downloaded_sra <- TRUE
  ...
  if (!downloaded_sra) {
    ...
    if (!is.null(sra_https_link)) {
      ...
      tryCatch({
        ...
        execute_download(url = sra_https_link, dest_file = dest_file,
                         parallel_n = parallel_n, software_paths = software_paths)
        if (fs::file_exists(dest_file) && fs::file_size(dest_file) > 0) {
          downloaded_sra <- TRUE
        } else {
...
download_gsa_run <- function(run_id, run_info, use_aspera = FALSE, parallel_n = 0,
  ...
  for (link in links_to_try) {
    ...
    tryCatch({
      if (download_mode == "Aspera") {
        execute_aspera(remote_path = link, db = "GSA", software_paths = software_paths,
                       aspera_key_path = aspera_key_path, output_dir = output_dir)
      } else { # FTP, HTTPS, Huawei Cloud
        execute_download(url = link, dest_file = dest_file,
                         parallel_n = parallel_n, software_paths = software_paths)
      }
      if (!fs::file_exists(dest_file) || fs::file_size(dest_file) == 0) {
        stop("Download resulted in missing or empty file.")
